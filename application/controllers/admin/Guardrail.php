...
class Guardrail extends CI_Controller{
	...
	public function index(){
		...
		if (isset($cari)) {
			...
			$sdel 		= $this->guardrail_model->guardrailbykd($cari);
			$kd_jalan	= $sdel[0]->kd_jalan;
			$jln 		= $this->guardrail_model->jlnprovbykd($kd_jalan);
			...
			$data = array(	'title' 	=> 'View Guardrail',
							'ruas'		=> $ruas,
							'view'		=> $sdel,
							'isi' 		=> 'admin/guardrail/view_search');
   ...
   public function kml(){
	...
	public function cetak($kd){
		...
		$data = array('nama'			=> 'Guardrail',
							'guardrail'	=> $guardrail,
							'jalan' 		=> $jalan);
		$this->load->view('admin/guardrail/print', $data);
	}
