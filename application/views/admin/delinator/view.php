...
<div class="content-wrapper">
   <section class="content-header">
      <h1>Delinator <small><?php echo $ruas->nm_ruas.' ( '.angka($ruas->jln_panjang).' m)' ?></small></h1>
      <ol class="breadcrumb">
         <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
         <li class="active">View Delinator</li>
      </ol>
   </section>
...
function myMap() {
   ...
   var ctaLayer = new google.maps.KmlLayer({
      url: '<?php echo base_url();?>'+'admin/delinator/jalanprovinsi/'+<?php echo $ruas->kd_jalan ?>+'?dummy='+(new Date()).getTime(),
      map: map
   });
      ...
      <?php foreach ($view as $key => $view): ?>
         {
            ...
            kode: '<?php echo $view->kd_delinator ?>',
            ruas: '<?php echo $view->nm_ruas ?>',
            jenis: '<?php if($view->jenis != null){echo $view->jenis;}else{echo "-";}?>',
            letak: '<?php echo $view->letak ?>',
            status: '<?php echo $view->status ?>',
            image: '<?php if($view->img_delinator != null){echo base_url('assets/upload/delinator/thumbs/'.$view->img_delinator);}else{echo base_url('assets/theme/img/map-marker-logo.jpg');} ?>'
         },
      <?php endforeach ?>
   ];
