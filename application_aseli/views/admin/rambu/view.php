...
<div class="content-wrapper">
   <section class="content-header">
      <h1>Rambu <small><?php echo $ruas->nm_ruas.' ( '.angka($ruas->jln_panjang).' m)' ?></small></h1>
      <ol class="breadcrumb">
         <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
         <li class="active">View Rambu</li>
      </ol>
   </section>
...
function myMap() {
   ...
   var ctaLayer = new google.maps.KmlLayer({
      url: '<?php echo base_url();?>'+'admin/rambu/jalanprovinsi/'+<?php echo $ruas->kd_jalan ?>+'?dummy='+(new Date()).getTime(),
      map: map
   });
      ...
      <?php foreach ($view as $key => $view): ?>
         {
            ...
            kode: '<?php echo $view->kd_rambu ?>',
            jenis: '<?php if($view->jenis != null){echo $view->nm_perjal;}else{echo "-";}?>',
            letak: '<?php echo $view->posisi ?>',
            status: '<?php echo $view->status ?>',
            image: '<?php echo base_url('assets/upload/img_rambu/'.$view->img_tipe) ?>',
            imagezoom: '<?php echo base_url('assets/upload/rambu/'.$view->img_rambu) ?>'
         },
      <?php endforeach ?>
   ];
